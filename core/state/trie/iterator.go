...
type nodeIteratorState struct {
	hash    etypes.Hash // Hash of the node being iterated (nil if not standalone)
	node    node        // Trie node being iterated
	parent  etypes.Hash // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() etypes.Hash {
	if len(it.stack) == 0 {
		return etypes.Hash{}
	}
...
func (it *nodeIterator) Parent() etypes.Hash {
	if len(it.stack) == 0 {
		return etypes.Hash{}
	}
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(hashNode); ok {
		...
		st.node = resolved
		st.hash = etypes.HashSetBytes(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor etypes.Hash) (*nodeIteratorState, []byte, bool) {
...
func compareNodes(a, b NodeIterator) int {
	...
	aHash := a.Hash()
	bHash := b.Hash()
	if cmp := bytes.Compare(aHash.Bytes(), bHash.Bytes()); cmp != 0 {
		return cmp
	}
	if a.Leaf() && b.Leaf() {
...
func (it *differenceIterator) Hash() etypes.Hash {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() etypes.Hash {
	return it.b.Parent()
}
...
func (it *unionIterator) Hash() etypes.Hash {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() etypes.Hash {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == etypes.Hash{}) {
			it.count++
